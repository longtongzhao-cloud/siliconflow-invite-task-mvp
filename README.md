...
当前仓库不能直接用于生产：SiliconFlow 真实代理登录、淘宝订单/消息权限和支付宝自动付款均未接通。本站已实现阿里云 PNVS 短信认证适配器及受控测试保护，但账号开通、凭据配置和真实发送尚未现场通过；开发环境默认仍使用不联网的 mock 或人工适配器。
...
个人开发者开通阿里云短信认证及 WSL 真发验收步骤见 [阿里云短信认证接入与验收](mvp/ALIYUN_SMS_SETUP.md)。
